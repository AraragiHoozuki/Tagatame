...
Yanfly.Icon.RemoveEquip = String(Yanfly.Parameters['Remove Icon']);
Yanfly.Param.EquipEmptyText = String(Yanfly.Parameters['Empty Text']);
Yanfly.Icon.EmptyEquip = String(Yanfly.Parameters['Empty Icon']);
...
Window_EquipItem.prototype.drawRemoveEquip = function(index) {
    ...
    this.drawText(text, rect.x + ibw, rect.y+24, rect.width - ibw);
};
...
Scene_Equip.prototype.isLowerWindowTouched = function() {
    ...
    return false;//(x >= rect.x && y >= rect.y && x < rect.width && y < rect.height);
};
