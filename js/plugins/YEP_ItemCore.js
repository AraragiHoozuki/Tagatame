...
//=============================================================================
 /*:
 * @plugindesc v1.27 Changes the way Items are handled for your game
...
Window_ItemCategory.prototype.windowWidth = function() {
    if (SceneManager._scene instanceof Scene_Item) return Graphics.boxWidth;
...
Window_ItemList.prototype.drawItemNumber = function(item, x, y, width) {
    ...
    this.contents.fontSize = Yanfly.Param.ItemQuantitySize;
    this.drawText('\u00d7' + numItems, x, y+32, width, 'right');
    this.resetFontSettings();
};
...
Window_ItemInfo.prototype.refresh = function() {
    ...
    dy = this.drawTooltip(dy);
    dy = this.drawBonusStatus(dy)
...
Window_ItemInfo.prototype.drawTooltip = function(dy) {
	var dx = this.textPadding();
	var dw = this.contents.width - this.textPadding() * 2;
	this.resetTextColor();

	var ex;
	if (DataManager.isWeapon(this._item)) {ex = $dataWeaponsEx;}
	if (DataManager.isArmor(this._item)) {ex = $dataArmorsEx;}
	if (DataManager.isItem(this._item)) {ex = $dataItemsEx;}
	if (!ex) {return dy;}
	var itemEx = ex[this._item.id];

	if (itemEx&&itemEx.tooltip) {
		this.drawText(itemEx.tooltip, dx, dy, dw);
		return dy + this.lineHeight();
	}
	return dy;
};

Window_ItemInfo.prototype.drawBonusStatus = function(dy) {
	var dx = this.textPadding();
	var dw = this.contents.width - this.textPadding() * 2;
	this.resetTextColor();
	...
	if (itemEx&&itemEx.bonus) {
		for (var key in itemEx.bonus) {
			this.drawText(key + "+" + itemEx.bonus[key], dx, dy, dw);
			dy = dy + this.lineHeight();
		}
	}
	return dy;
};
